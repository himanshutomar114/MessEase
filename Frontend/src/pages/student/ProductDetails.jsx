...
const ProductDetails = () => {
  ...
  const handleDeleteReview = async (reviewId) => {
    try {
      await api.delete(`/api/marketplace/products/${id}/reviews/${reviewId}`, {
        withCredentials: true,
      });
      fetchReviews();
    } catch (error) {
      ...
      const options = {
        ...
        handler: async function (razorpayResponse) {
          try {
            ...
            await api.post(
              `/api/marketplace/verify-payment`,
              {
              ...
              {reviews.length > 0 ? (
                <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                  {reviews.map((review) => (
                    <div
                      key={review._id}
                      className="bg-gray-700 p-4 rounded-lg transition-all hover:bg-gray-650"
                    >
