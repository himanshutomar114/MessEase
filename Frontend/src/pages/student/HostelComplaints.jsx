...
const HostelComplaints = () => {
  ...
  useEffect(() => {
    const fetchComplaints = async () => {
      setIsLoading(true);
      try {
        const response = await api.get(`/api/complaint/getcomplaints/${hostelCode}`);
        // Sort complaints by date (newest first)
        const complaints = response.data.complaint || [];
        const sortedComplaints = complaints.sort(
          (a, b) => new Date(b.createdAt) - new Date(a.createdAt)
        );
        setHostelComplaints(sortedComplaints);
      } catch (err) {
    ...
    fetchComplaints();
  }, [hostelCode, success]); // Re-fetch when success changes (after new submission)
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <textarea
                  ...
                  placeholder="Describe your hostel complaint in detail"
              ...
              <div className="text-center bg-[#0A0A1A] border border-[#2A2A4A] rounded-lg p-8 py-16">
                ...
                <div className="border-t border-[#2A2A4A] pt-6 mt-6">
                  <p className="text-sm text-gray-500">
                    All submitted hostel complaints will appear here
                  </p>
                </div>
              </div>
            ) : (
